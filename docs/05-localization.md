...
The strings are generated by the Flutter Intl VS Code extension rather
than plain `flutter gen-l10n`: it watches the ARB files and rewrites
`lib/generated/` on save.

The generated `S` class lives there, and the analyzer flagged it heavily
— `intl` imports, missing type annotations, lint rules the generator
doesn't follow. None of that is fixable by hand, since the whole folder
