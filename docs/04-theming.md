...
Changing a single color means hunting it down in every file that uses
it, and reusing a style means copying it, so the "same" style slowly
drifts apart across screens. A second theme — a dark one, say — isn't
possible without editing every widget in the app.
